...
describe('Integration tests for /v1/user endpoint', () => {
  it('Test 1: Create an account and validate existence using GET', async function () {
    this.timeout(10000); // Set timeout to 5000ms (5 seconds)
    const newUser = {
      first_name: 'John',
      last_name: 'Doe',
    ...
    expect(createResponse.statusCode).to.equal(201); // Check for successful creation
        ...
        // Update the created user to set verified to true
        await User.update({ verified: true }, { where: { username: newUser.username } });
    ...
    expect(getUserResponse.body.verified).to.equal(true); // Check if user is verified
  });
